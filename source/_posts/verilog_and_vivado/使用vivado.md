...
![](../../figure/verilog_and_vivado/使用vivado/project_name.webp)

选择项目类型：
RTL Project 能自主编辑source，创建、修改IP，支持各种功能。
![](../../figure/verilog_and_vivado/使用vivado/project_type.webp)

选择板子型号（应该是）：
![](../../figure/verilog_and_vivado/使用vivado/default_part.webp)
...
![](../../figure/verilog_and_vivado/使用vivado/overall_view.webp)

## 添加源文件
可以在左侧 Flow Navigator->PROJECT MANAGER->Add Scource处或者在中间的Sources下按“+”号。
![](../../figure/verilog_and_vivado/使用vivado/add_source_file.webp)
然后可以选择添加文件的类型
![](../../figure/verilog_and_vivado/使用vivado/add_source_cat.webp)
* constraints应该是管脚约束文件
* design source文件就是写Verilog的地方
* simulation source也是创建.v文件，但这个应该还与run simulation有关

![](../../figure/verilog_and_vivado/使用vivado/add_source_choice.webp)
然后可以选择从其他地方添加文件、文件夹，或者直接新建文件。

## 代码编辑
双击左侧的source中的文件，就能在右侧的编辑器中打开文件。
![](../../figure/verilog_and_vivado/使用vivado/code_edit.webp)
同时这些生成的文件路径就在项目文件夹的.srcs文件夹中。可以用其他文本编辑器打开文件进行编辑，保存后在vivado中也会显示。
![](../../figure/verilog_and_vivado/使用vivado/file_path.webp)

## 代码仿真(Simulation)
点击SIMUALTION->Run Simulation->Behavioral Simulation就可以展示图右图simulation的窗口。
![](../../figure/verilog_and_vivado/使用vivado/run_simulation.webp)
...
![](../../figure/verilog_and_vivado/使用vivado/synthesis_completed.webp)

完成后在右上角选择I/O Planning，下方会出现如图管脚信息。然后选择Package Pin和I/O Std完成设置。点击保存，即可保存管脚信息。
![](../../figure/verilog_and_vivado/使用vivado/IO_prots_gui.webp)
### 通过代码添加
