...
* 数据同步与备份：支持增量备份 ，只传输发生变化的文件，节省时间和带宽。此外还支持加密传输 、断点续传 。
* 挂载功能：能将远程存储（如 WebDAV、S3、Google Drive 等）挂载到本地目录，使其像本地文件系统一样工作。这一点类似 RaiDrive。

关于 Rclone 的使用，可以参考[这篇文章](https://zhuxz0299.github.io/posts/811636f.html)。