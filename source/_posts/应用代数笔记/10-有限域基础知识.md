...
&\iff x^{q^{d}-1} -1 \mid x^{q^{n}-1} -1 \iff x^{q^{d}} -x \mid x^{q^{n}}-x \\
...
### 本原多项式
> **定义 (本原多项式)**
> 如果 $\alpha$ 是 $F_{q^n}^*$ 的**生成元（本原元）**，则 $ord(\alpha) = q^n - 1$。此时 $\alpha$ 在 $F_q$ 上的极小多项式称为**本原多项式 (Primitive Polynomial)**。本原多项式的阶为 $q^n - 1$。
