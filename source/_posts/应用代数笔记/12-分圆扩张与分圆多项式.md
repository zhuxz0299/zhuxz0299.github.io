...
> **定义：本原单位根**
> $U_n$ 的生成元称为 **$1$ 的 $n$ 次本原根**。记 $\omega_n$ 为 $1$ 的某一个本原 $n$ 次根，则有：
...
3.  **但是**，依然有 $S = \mathbb{F}_2(\alpha^3) = \mathbb{F}_{16}$。
    *(这说明即便元素不是 $n$ 次本原根，它生成的域也可能等于整个分圆扩张域)*。
...
**证明思路：**
引入映射 $\kappa: \sigma \mapsto k(\sigma)$，满足 $\sigma(\omega_n) = \omega_n^{k(\sigma)}$。
1.  **同态性**：$\sigma(\tau(\omega_n)) = \sigma(\omega_n^{k(\tau)}) = (\omega_n^{k(\sigma)})^{k(\tau)} = \omega_n^{k(\sigma)k(\tau)}$，对应于群乘法。
2.  **单射性**：若 $\kappa(\sigma) = 1$，则 $\sigma(\omega_n) = \omega_n$，即 $\sigma$ 固定生成元，故 $\sigma = \text{id}$。

**注意**：$\kappa$ 不一定是满射。
* 在 $F = \mathbb{Q}$ 时，$\kappa$ 是同构，即 $|\text{Gal}(S/F)| = \phi(n)$。
* 在有限域 $F_q$ 上，往往只是真子群。
...
对于 $n$ 的任意因子 $d$ ($d \mid n$)，记 $\omega_d$ 为 $d$ 次本原根，即 $\text{ord}(\omega_d) = d$。
考虑所有 $d$ 次本原根的集合，形式为 $\omega_d^k$，其中 $(k, d) = 1, 1 \le k < d$。这个集合的大小为 $\phi(d)$。

> **定义：分圆多项式**
> 令 $Q_d(x)$ 为以所有 $d$ 次本原根为根的多项式：
...
回顾之前建立的单射同态 $\kappa: \text{Gal}(S/F) \to \mathbb{Z}_n^*$。
我们已知 $|\mathbb{Z}_n^*| = \phi(n)$。

**推导过程：**
1.  **假设同构**：如果 $|\text{Gal}(S/F)| = \phi(n)$，由于 $\kappa$ 是单射且两边阶数相等，则 $\kappa$ 必为**满射**，即 $\text{Gal}(S/F) \cong \mathbb{Z}_n^*$。
...
**1. 设定与群结构**
设 $F = \mathbb{Q}$，$\omega$ 为 $7$ 次本原单位根，$E = \mathbb{Q}(\omega)$。
