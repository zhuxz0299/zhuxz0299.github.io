...
```vim
:[范围]s[分隔符]查找内容[分隔符]替换内容[分隔符][选项]
```

#### 范围

“范围”决定命令对哪些行生效。如果省略范围，则只处理光标所在的当前行。

| 范围 | 含义 |
| :--- | :--- |
| 省略 | 当前行 |
| `%` | 整个文件 |
| `10,20` | 第 10 至 20 行，包含起止行 |
| `.,$` | 从当前行到文件末尾，`.` 表示当前行，`$` 表示最后一行 |
| `'<,'>` | Visual 模式选中的行范围；选中内容后按 `:` 会自动填入 |

#### 分隔符

分隔符用来分开“查找内容”、“替换内容”和“选项”。`/` 并不是固定分隔符，可以换成大多数单字节的非字母数字字符。例如：
...
当查找或替换的内容本身包含大量 `/` 时，用别的分隔符可以避免频繁输入转义符 `\`。例如，下面两条命令效果相同，但第二条更容易阅读：

```vim
:%s/\/home\/user/\/opt\/app/g
:%s#/home/user#/opt/app#g
```

#### 选项

“选项”放在最后一个分隔符之后，用来控制替换方式。多个选项可以组合使用，例如 `gc` 表示“替换每行中的所有匹配，并在每次替换前确认”。
...
使用 `c` 选项时，常用的确认按键有 `y`（替换当前项）、`n`（跳过当前项）、`a`（替换后面的所有匹配项）和 `q`（退出）。
...
| `:%s/old/new/gc` | 在整个文件中替换所有 `old`，每次替换前都进行确认 |
| `:10,20s/old/new/gi` | 在第 10 至 20 行中替换所有 `old`，并忽略大小写 |
| `:'<,'>s/^/# /g` | 在 Visual 模式选中的每行行首添加 `# ` |
