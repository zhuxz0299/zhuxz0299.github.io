...
在打字的时候如果需要移动光标，把手指挪到方向键的位置很不方便，但是 Capslock 键在很好按到的位置。vim 虽然是一个很好的选择，但是学习 vim 需要改变一些原有的打字习惯，以及 vim 的操作逻辑总是要在各种文本编辑器软件中实现，无法做到全局操作。Windows 中的 Capslock+ 以及 Linux 系统中的 kedy 利用 Capslock 组合键解决了这个问题。
